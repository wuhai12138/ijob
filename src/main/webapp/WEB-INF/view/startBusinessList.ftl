                ...
                <div class="ibox float-e-margins">
                    <div class="ibox-title" style="padding: 0px 20px;">
                        <h2>创业新干线
                            ...
                            <button class="btn btn-primary pull-right dim" onclick="exportBus()">导出</button>
                        </h2>
                    </div>
                                ...
                                <div class="col-sm-6">
                                    <div class="dataTables_filter" id="DataTables_Table_0_filter">
                                        <div class="input-group has-success" style="float: right;">
                                            <input class="form-control input-sm" id="search" style="height: 36px;" placeholder="请输入标题进行搜索" <#if param.search??>value="${param.search}"</#if>>
                                ...
                                <#if infoList?exists>
                                    ...
                                    <tr>
                                        ...
                                        <td style="padding-top: 8px; padding-bottom: 0px;" class="text-center">
                                            <button type="button" class="btn btn-warning btn-xs" onclick="showEnroll(${list.tid})">查看报名</button>
                                        </td>
                                        <td style="padding-top: 8px; padding-bottom: 0px;" class="text-center">
                                            <#--<button type="button" class="btn btn-primary btn-xs" onclick="changeStatus(${list.tid})">-->
                                                <#--<#if list.canEnroll == 0>-->
                                                    <#--隐藏-->
                                                <#--<#else>-->
                                                    <#--显示-->
...
//            var row = $("#row").val();
            var url = 'startBusinessDetail.do';
            layer.open({
                type: 2,
                title: '新增创业',
                maxmin: true,
        ...
        function updateOne(id) {
            var con = $('#add-new-template').html();
//            var row = $("#row").val();
            var url = 'startBusinessDetail.do?tid='+id;
            layer.open({
                type: 2,
                title: '更新创业',
                maxmin: true,
        ...
        function searchThis(){
            ...
            var row = $("#row").val();
            var url = "infoListPage.do?type=6&row="+row+"&search="+search;
        ...
        function goPage(id){
            var search = $("#search").val();
            var row = $("#row").val();
            var url = "infoListPage.do?type=6&row="+row+"&search="+search+"&currentPage="+id;
            self.location.replace(url);
        }
        function exportBus(){
            var url= "exportBus.do?type=6";
            window.open(url);
        }
    </script>
</body>
