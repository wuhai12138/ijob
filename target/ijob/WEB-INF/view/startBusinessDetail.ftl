    ...
    <form class="form-horizontal" id="form1">
        <div class="row">
            <div class="col-xs-12">
                ...
                <input id="type" name="type" <#if updateBus??>value="${updateBus.type!''}"<#else>value="${type!''}"</#if> hidden>
                    <label class="col-xs-3 control-label" style="text-align:right">标题：</label>
                    <div class="col-xs-6">
                        <input type="text" name="title" id="title" class="form-control" value="<#if updateBus??>${updateBus.title!''}</#if>">
                ...
                <div class="form-group">
                    ...
                    <div class="col-xs-6">
                        <input type="text" name="limitNum" id="limitNum" class="form-control" value="<#if updateBus??>${updateBus.limitNum!''}</#if>"/>
                        <span class="help-block m-b-none"></span>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-xs-6">
                        <textarea name="description" id="description" class="form-control"><#if updateBus??>${updateBus.description!''}</#if></textarea>
                        <span class="help-block m-b-none"></span>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-xs-6">
                        <input type="text" name="link" id="link" class="form-control" value="<#if updateBus??>${updateBus.link!''}</#if>">
                        <span class="help-block m-b-none"></span>
                    </div>
                </div>
                <div class="form-group">
                    ...
                    <div class="col-xs-6">
                        ...
                        <#if updateBus??>
                            <option value="0" <#if updateBus.canEnroll==0>selected</#if>>隐藏</option>
                            <option value="1" <#if updateBus.canEnroll==1>selected</#if>>显示</option>
                        <#else>
                ...
                <div class="form-group">
                    <label class="col-xs-3 control-label" style="text-align:right">分类：</label>
                    <div class="col-xs-6">
                        <select class="form-control" name="kind" id="kind">
                        <#if updateBus??>
                            <option value="0" <#if updateBus.kind==0>selected</#if>>默认</option>
                            <option value="1" <#if updateBus.kind==1>selected</#if>>广告</option>
                            <option value="2" <#if updateBus.kind==2>selected</#if>>精选</option>
                        <#else>
...
<script>
    ...
    <#if updateBus??>
        var url = "updateInfo.do?tid=${updateBus.tid}"+"&title="+title+"&description="+description+"&link="+link
                +"&type="+type+"&canEnroll="+canEnroll+"&kind="+kind+"&limitNum="+limitNum;
    <#else>
